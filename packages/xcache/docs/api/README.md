**@httpx/xcache v0.4.10**

---

# @httpx/xcache v0.4.10

## Classes

- [CacheCompress](classes/CacheCompress.md)
- [DevalueSerializer](classes/DevalueSerializer.md)
- [JsonSerializer](classes/JsonSerializer.md)
- [SuperjsonSerializer](classes/SuperjsonSerializer.md)
- [TimeLruCache](classes/TimeLruCache.md)
- [XMemCache](classes/XMemCache.md)

## Interfaces

- [ICacheCompressor](interfaces/ICacheCompressor.md)
- [ICacheSerializer](interfaces/ICacheSerializer.md)
- [XCacheRunAsyncParams](interfaces/XCacheRunAsyncParams.md)

## Type Aliases

- [CacheableAsyncFunction](type-aliases/CacheableAsyncFunction.md)
- [XMemCacheOptions](type-aliases/XMemCacheOptions.md)

## Functions

- [createCacheFn](functions/createCacheFn.md)
