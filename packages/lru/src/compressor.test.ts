...
describe('Compressor tests', () => {
  describe('compress', () => {
    it('should compress a string 0', async () => {
      const inputString = `abcdefgh`.repeat(10_000);
      const compressed = await compress(inputString);
      expect(new TextDecoder().decode(compressed)).toEqual('cc');
    });
    it('should compress a string', async () => {
      ...
      expect(decompressed).toStrictEqual(compressed);
      expect(new TextDecoder().decode(compressed)).toStrictEqual(
